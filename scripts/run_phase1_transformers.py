...
def main():
    ...
    # --- pre-flight: fail before spawning workers / loading models -------------
    ...
    chunks = preflight.shard(pages, len(gpu_ids))  # exactly len(gpu_ids) buckets, some maybe empty
    print(f"[info] {len(pages)} pages across GPUs {gpu_ids}: {[len(c) for c in chunks]}", flush=True)

    with runner.acquire_run_lock(args.pred_dir, command=["run_phase1", "transformers", str(args.pred_dir)]):
        ...
        # run_counts: only pages this run gave a definitive outcome (succeeded+failed);
        # skipped is derived so expected == attempted + skipped holds even when a
        # worker crash leaves pages pending (captured by final_state.pending).
        run_succeeded = counts["complete"]
        run_failed = counts["failed"]
        run_attempted = run_succeeded + run_failed
        runner.write_run_manifest(
            ...
            run_counts={
                ...
                "failed": run_failed,
                "skipped": len(pages) - run_attempted,
            },
            final_state={
                "expected": len(pages),
