...
## Part 3 — RC3 upstream candidate finalization (2026-08-30, upstream `2c7d7dd64a`)

The RC3 fix (forward only present RoPE grid kwargs) was isolated as the first upstream candidate after human review approved the approach: local branch `bugfix/transformers-rope-grid-kwargs`, single commit `cd711d5fa68d639b4121056da66c5c60c9895459` (author `AIwork4me <AIwork4me@qq.com>`, `Co-authored-by: ZCode Agent`, DCO `Signed-off-by`), +102/−2 across exactly two files. Test scope reduced per vLLM AGENTS.md minimal-diff guidance: 146 → 95 lines, two behaviors — `test_omits_absent_grid_kwargs` (regression: pristine `2c7d7dd64a` FAILS with `TypeError: … 'video_grid_thw'`, candidate PASSES, asserts image grid + `mm_token_type_ids` forwarding and valid positions) and `test_forwards_present_image_grid` (sentinel guard distinguishing omitted vs passed-None vs real tensor). The video-gate pin test was removed (it pinned an unrelated existing limitation). Test location kept as a dedicated CPU file — `test_backend.py` is hub/GPU integration throughout and would couple these unit tests to its fixtures. Final proof pair: `latest-main/rc3-final-before.log` (2 FAILED incl. the exact TypeError) → `rc3-final-after.log` (2 PASSED); surrounding mrope executor suites 11/11 PASS; ruff check/format clean; `git diff --check` clean; pre-commit not installable offline (uv unavailable; pinned ruff 0.14.0 from the project venv used instead). Real-model boundary evidence (unchanged from Part 2, logs `rc3-boundary-before/after.log`): with RC1/RC2 as declared local prerequisites, the real HunyuanOCR request fails at exactly `TypeError: … 'video_grid_thw'` before and completes correct OCR after — RC3 was the only new variable. Duplicate-work search at finalization: still no overlapping PR. `rc3-pr-draft.md` holds the PR text; `rc3-final-candidate.diff` is the exact two-file commit patch.
