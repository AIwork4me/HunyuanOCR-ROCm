...
def _maybe_cap_image(path: str, max_pixels: int | None) -> str:
    """If ``max_pixels`` is set and the image exceeds it, write a temporary
    longest-edge-thumbnail copy to stay under the ROCm ViT threshold and return
    its path; otherwise return the original path untouched."""
    if not max_pixels:
        return path
    ...
    with Image.open(path) as im:
        ...
        tmp = path + f".cap{max_pixels}.png"
        thumb.save(tmp)
        return tmp


def infer_one(client, image_path: str, prompt: str | None = None, *,
              model: str = "tencent/HunyuanOCR", max_tokens: int = 32768,
              repetition_penalty: float = 1.08, repeat_min_repeats: int = 8,
    ...
    common = dict(
        ...
        extra_body={"top_k": -1, "repetition_penalty": repetition_penalty,
